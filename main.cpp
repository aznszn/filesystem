...
void truncate(File* root, File* cwd,vector<opened_file*> open_files, vector<vector<char>>& disk, vector<bool>& free_blocks);
void move_within_file(File* root, File* cwd,vector<opened_file*> open_files, vector<vector<char>>& disk, vector<bool>& free_blocks);
    ...
    if(returned) {
        ...
        if (it != open_files.end() && (*it)->mode != "R" && (*it)->mode != "r"){
            ...
            if(new_file_mem.size()/BLOCK_SIZE > file_size_on_disk/BLOCK_SIZE){
                int new_blocks_required = ceil((double)new_file_mem.size()/BLOCK_SIZE) - file_size_on_disk/BLOCK_SIZE;
